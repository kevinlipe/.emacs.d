  ...
  #+END_SRC
   ...
   (let* ((packages '(
	   ; base16-theme         ; Base16 theme packages; we'll use "Eighties"
           color-theme-sanityinc-tomorrow ; new color theme
...
Use the Oxygen Mono font (which is installed on my systems) at different sizes depending on whether we're on OS X or on Linux. 

#+BEGIN_SRC emacs-lisp
  (cond ((member "Oxygen Mono" (font-family-list))
   (if (memq window-system '(mac ns))
    (set-face-attribute 'default nil :font "Oxygen Mono" :height 140) ;; on OS X
    (set-face-attribute 'default nil :font "Oxygen Mono" :height 105)) ;; on Linux
    ))
#+END_SRC
