...
* Packages
   ...
   loaded, along with =package=, which is obviously needed.
    ...
    (when (version< emacs-version "27") (package-initialize)) ; package-initialize is deprecated in Emacs 27
    #+END_SRC
    ...
    #+END_SRC
     ...
     (let* ((packages '(
	     ...
	     csv-mode             ; For working with .csv files intelligently
             racket               ; Major mode for Racket programming
...
I like to use the Base16 themes. So far =base16-ashes= is what I like, but it changes pretty regularly. In the terminal (including on Windows Subsystem for Linux), I like to use =wheatgrass= instead because it works much better there.
...
I like to use IBM Plex Mono as my coding font across all platforms, but it needs to be a different size depending on what OS I'm on because every one is special.
...
#+BEGIN_SRC emacs-lisp
   (if (memq window-system '(mac ns))
    ...
    (if (string-equal system-type "windows-nt") ;; if not Mac, check for Windows
      (set-face-attribute 'default nil :font "IBM Plex Mono" :height 120) ;; on Win
