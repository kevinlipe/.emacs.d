...
** Packages
  ...
  #+END_SRC
   ...
   (let* ((packages '(
	   ...
	   helm                 ; Incremental completion mode
...
I want the Escape key to quit whatever I'm doing in the minibuffer.
...
#+BEGIN_SRC emacs-lisp
  (cond ((member "Oxygen Mono" (font-family-list))
   (if (memq window-system '(mac ns))
    ...
    (set-face-attribute 'default nil :font "Oxygen Mono" :height 110)) ;; on Linux
    ))
#+END_SRC
...
Here's where we configure some stuff with helm (I don't know what I'm doing with this yet)
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; use C-x f for helm finding, as opposed to normal C-x C-f
