...
Windows being what it is, there are configurations we need to make to make Emacs (the emax version) behave in a Windows environment.
...
#+BEGIN_SRC emacs-lisp
 ...
 ;;make sure we have the latest org
  (use-package org
    ;;:pin org
