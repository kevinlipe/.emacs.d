...
*** =RET= vs. =kp-enter=
    ...
    On some of my laptops, hitting =M-return= actually sends a =kp-enter= to Emacs, which means some of my Org default keybindings (specifically =M-RET= and =M-S-RET=) stop working and I get a "=M-enter is undefined=" message. This binds =kp-enter= to do the same things so that I don't have those problems anymore.

#+BEGIN_SRC emacs-lisp

;  TODO: Figure out a cleaner way to implement this that doesn't break the rest of the configuration.
