...
I like the =plan9= and =acme= color schemes, depending on my mood. Both are based on Plan 9 and its =acme= editor, but one is a little better for Org mode stuff and one is a little better for code editing and things like that. I switch between them liberally. For now, =acme= is the default.
...
#+BEGIN_SRC emacs-lisp
(if (window-system) (load-theme 'acme t nil) ;; (window-system) returns nil if emacs isn't running in one.
      (load-theme 'wheatgrass t nil)) ;; load wheatgrass if we can't use the base16 theme.

(when (window-system) (load-theme 'plan9 t t)) ;; load the acme theme but don't enable it (only if there's a window system).
#+END_SRC
